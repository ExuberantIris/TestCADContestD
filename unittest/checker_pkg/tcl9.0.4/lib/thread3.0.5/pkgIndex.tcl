...
if {[package vsatisfies [package provide Tcl] 9.0-]} {
    package ifneeded thread 3.0.5 \
	    [list load [file join $dir libtcl9thread3.0.5.so] [string totitle thread]]
} else {
    package ifneeded thread 3.0.5 \
	    [list load [file join $dir libthread3.0.5.so] [string totitle thread]]
}
package ifneeded [string totitle thread] 3.0.5 \
    [list package require -exact thread 3.0.5]

# package ttrace uses some support machinery.

package ifneeded ttrace 3.0.5 [list ::apply {{dir} {
	...
	source [file join $dir ttrace.tcl]
	    ...
	    || ![catch {zipfs mount [file join $dir libtcl9thread3.0.5.so] //zipfs:/lib/thread}]} {
	source //zipfs:/lib/thread/ttrace.tcl
    } elseif {[file exists //zipfs:/app/thread_library/ttrace.tcl]
	    || ![catch {zipfs mount [file join $dir libtcl9thread3.0.5.so] //zipfs:/app/thread_library}]} {
...
package ifneeded Ttrace 3.0.5 \
    [list package require -exact ttrace 3.0.5]
